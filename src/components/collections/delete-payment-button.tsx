...
export function DeletePaymentButton({ paymentId }: { paymentId: string }) {
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      toast.success("Abono eliminado, saldo actualizado");
      setOpen(false);
      router.refresh();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "No se pudo eliminar el abono");
    } finally {
      setLoading(false);
    }
  };
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            El saldo de la cobranza se recalculará sin este pago. Esta acción no
            se puede deshacer.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
